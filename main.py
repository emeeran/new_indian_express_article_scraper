...
def sanitize_filename(filename):
    """
    Sanitizes the given filename by removing any characters that are not allowed in a filename.
...
def scrape_article(url):
    ...
    Returns:
        tuple: A tuple containing the markdown content and the headline of the article.
    """
    ...
    date = (
        date_element.text.strip() if date_element else "Date information not available"
    )
    ...
    content_elements = soup.select(
        f"p:not({excluded_selectors})"
    )  # Direct exclusion using CSS selector
...
def main():
    """
    Main function that prompts the user for the URL of the article, scrapes the article content,
